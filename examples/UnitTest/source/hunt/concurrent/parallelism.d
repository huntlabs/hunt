/**
`hunt.concurrent.parallelism` implements high-level primitives for SMP parallelism.
These include parallel foreach, parallel reduce, parallel eager map, pipelining
and future/promise parallelism.  `hunt.concurrent.parallelism` is recommended when the
...
`hunt.concurrent.parallelism` is based on the concept of a `Task`.  A `Task` is an
...
*/
module hunt.concurrent.parallelism;
...
@system unittest
{
    ...
    import hunt.concurrent.parallelism : taskPool;
...
struct AbstractTask
{
    ...
    private void function(void*) taskRun;
    // private void function(void*) taskDone;

    private Throwable exception;
    private ubyte taskStatus = TaskStatus.notStarted;
...
alias TaskHandler = void delegate();
alias TaskRejectedHandler = void delegate(Throwable);
...
{
    AbstractTask base = {taskRun : &onTaskRun};
    alias base this;
    ...
    private static void onTaskRun(void* myTask)
    {
        ...
        static if (is(ReturnType == void))
        {
            ...
            myCastedTask.resolve();
        }
        else static if (is(typeof(addressOf(fun(myCastedTask._args)))))
        {
            myCastedTask.resolve(addressOf(fun(myCastedTask._args)));
        }
        else
        {
            myCastedTask.resolve(fun(myCastedTask._args));
        }
    }
    ...
    { 
        ...
        if(this.exception is null) {
            this.doneWithSuccess();
        } else {
            this.doneWithFailure();
        }
    }
    ...
    */
    ...
    static if (!is(ReturnType == void))
    {
        ...
        private void doneWithSuccess() {
            if(_fulfilledHandler !is null) 
                _fulfilledHandler(this.returnVal);
        }
        alias TaskFulfilledHandler = void delegate(ReturnType);

        void resolve(ReturnType value) {
            if(atomicReadUbyte(taskStatus) != TaskStatus.done) {
                returnVal = value;
                onTaskDone();
            // } else {
                // ConsoleLogger.warning("resolved");
            }
        }
    }
    else 
    {
        ...
        alias TaskFulfilledHandler = TaskHandler;
    ...
    private void doneWithFailure() {
        if(_rejectedHandler !is null) 
            _rejectedHandler(this.exception);
    }

    void cancel() {
        
        enforcePool();
        if(!this.pool.isSingleTask)
            this.pool.deleteItem(basePtr);

        if(atomicReadUbyte(taskStatus) == TaskStatus.done) {
                ConsoleLogger.warning("cancelled");
        } else {
            this.exception = new TaskCancelledException("task cancelled");
            onTaskDone();
        }
    }
    ...
    @property ref ReturnType workForce() @trusted
    {
        ...
        while (true)
        {
            if (doneWithCheck)    // done() implicitly checks for exceptions.
            {
                static if (is(ReturnType == void))
                {
                    return;
    ...
    */
    ...
    private @property bool doneWithCheck() @trusted
    {
...
1.  It distinguishes hunt.concurrent.parallelism threads from other threads so that
    the hunt.concurrent.parallelism daemon threads can be terminated.
            ...
            else
            {
                ...
                if (!ptr)
                {
                    throw new OutOfMemoryError(
                        "Out of memory in hunt.concurrent.parallelism."
                    );
                }
...
)
{
    ...
    else
    {
        ...
        if (!ptr) throw new OutOfMemoryError("Out of memory in hunt.concurrent.parallelism.");
