...
module hunt.concurrent.ForkJoinPool;
...
// import hunt.concurrent.locks.LockSupport;
 ...
 * <ul>
 * <li>{@code hunt.concurrent.ForkJoinPool.common.parallelism}
 * - the parallelism level, a non-negative integer
 * <li>{@code hunt.concurrent.ForkJoinPool.common.threadFactory}
 * - the class name of a {@link ForkJoinWorkerThreadFactory}.
 * The {@linkplain ClassLoader#getSystemClassLoader() system class loader}
 * is used to load this class.
 * <li>{@code hunt.concurrent.ForkJoinPool.common.exceptionHandler}
 * - the class name of a {@link UncaughtExceptionHandler}.
 * The {@linkplain ClassLoader#getSystemClassLoader() system class loader}
 * is used to load this class.
 * <li>{@code hunt.concurrent.ForkJoinPool.common.maximumSpares}
...
class ForkJoinPool { // : AbstractExecutorService 
    ...
    //     int phase;        // versioned, negative: queued, 1: locked
    ...
    //         int s = top, d, cap, m;
    //         ForkJoinPool p = pool;
    //         if ((a = array) !is null && (cap = a.length) > 0) {
    ...
    //      */
    //     final bool lockedPush(ForkJoinTask<?> task) {
    //         ForkJoinTask<?>[] a;
    //         bool signal = false;
    ...
    //             if ((oldA = array) !is null && (oldSize = oldA.length) > 0 &&
    ...
    //         while ((a = array) !is null && (cap = a.length) > 0 &&
    //                top - (b = base) > 0) {
    ...
    //             if ((md & FIFO) == 0 || d == 1) {
    //                 if ((t = (ForkJoinTask<?>)
    //                      QA.getAndSet(a, (cap - 1) & --s, null)) !is null)
    //                     TOP.setOpaque(this, s);
    //             }
    //             else if ((t = (ForkJoinTask<?>)
    //                       QA.getAndSet(a, (cap - 1) & b++, null)) !is null) {
    //                 BASE.setOpaque(this, b);
    //             }
    ...
    //         return ((a = array) !is null && (cap = a.length) > 0) ?
    ...
    //      */
    //     final bool tryUnpush(ForkJoinTask<?> task) {
    //         bool popped = false;
    ...
    //      */
    //     final bool tryLockedUnpush(ForkJoinTask<?> task) {
    //         bool popped = false;
    ...
    //         for (ForkJoinTask<?> t; (t = poll()) !is null; )
    ...
    //      */
    //     final int helpCC(CountedCompleter<?> task, int limit, bool shared) {
    //         int status = 0;
    //         if (task !is null && (status = task.status) >= 0) {
    ...
    //                 if (v !is null)
    //                     v.doExec();
    //                 if ((status = task.status) < 0 || v is null ||
    //                     (limit != 0 && --limit == 0))
    //                     break;
    //             }
    //         }
    ...
    //  * "hunt.concurrent.ForkJoinPool.common.maximumSpares" system
    ...
    //         if (ws is null || (n = ws.length) <= 0 || w is null)
    //             return 0;                        // disabled
    ...
    //     WorkQueue[] ws; int n;
    //     if ((ws = workQueues) !is null && (n = ws.length) > 0 && w !is null) {
    //         for (int m = n - 1, j = r & m;;) {
    //             WorkQueue q; int b;
    //             if ((q = ws[j]) !is null && q.top != (b = q.base)) {
    //                 int qid = q.id;
    //                 ForkJoinTask<?>[] a; int cap, k; ForkJoinTask<?> t;
    //                 if ((a = q.array) !is null && (cap = a.length) > 0) {
    //                     t = (ForkJoinTask<?>)QA.getAcquire(a, k = (cap - 1) & b);
    ...
    //         while (s >= 0) {
    //             WorkQueue[] ws;
    //             int n = (ws = workQueues) is null ? 0 : ws.length, m = n - 1;
    //             while (n > 0) {
    //                 WorkQueue q; int b;
    ...
    //                             t !is null && QA.compareAndSet(a, k, t, null)) {
    //                             q.base = b;
    //                             w.source = qid;
    //                             t.doExec();
    //                             w.source = src;
    ...
    //         int n = (ws = workQueues) is null ? 0 : ws.length;
    ...
    //  */
    // private ForkJoinTask<?> pollScan(bool submissionsOnly) {
    //     WorkQueue[] ws; int n;
    ...
    //         int md = mode, n;
    //         WorkQueue[] ws = workQueues;
    //         if ((md & SHUTDOWN) != 0 || ws is null || (n = ws.length) <= 0)
    //             throw new RejectedExecutionException();
    ...
    //                     if ((vs = workQueues) !is null && (vn = vs.length) > 0 &&
    ...
    //  */
    // final bool tryExternalUnpush(ForkJoinTask<?> task) {
    //     int r = ThreadLocalRandom.getProbe();
    ...
    //     int r = ThreadLocalRandom.getProbe();
    //     WorkQueue[] ws; WorkQueue w; int n;
    //     return ((ws = workQueues) !is null && (n = ws.length) > 0 &&
    //             (w = ws[(n - 1) & r & SQMASK]) !is null) ?
    ...
    //     if (((t = Thread.currentThread()) instanceof ForkJoinWorkerThread) &&
    //         (pool = (wt = (ForkJoinWorkerThread)t).pool) !is null &&
    ...
    //             ("hunt.concurrent.ForkJoinPool.common.parallelism");
    //         if (pp !is null)
    //             parallelism = Integer.parseInt(pp);
    //         fac = (ForkJoinWorkerThreadFactory) newInstanceFromSystemProperty(
    //             "hunt.concurrent.ForkJoinPool.common.threadFactory");
    //         handler = (UncaughtExceptionHandler) newInstanceFromSystemProperty(
    //             "hunt.concurrent.ForkJoinPool.common.exceptionHandler");
    ...
    //         (p = (wt = (ForkJoinWorkerThread)t).pool) !is null &&
    ...
    //                  (ws = p.workQueues) !is null && (n = ws.length) > 0)
    //             w = ws[(n - 1) & r & SQMASK];
    //         else
    //             w = null;
    //         if (w !is null)
    ...
    //             ("hunt.concurrent.ForkJoinPool.common.maximumSpares");
    //         if (p !is null)
