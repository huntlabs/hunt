...
// import hunt.concurrent.locks.AbstractQueuedSynchronizer;
// import hunt.concurrent.locks.Mutex;
...
class ThreadPoolExecutor : AbstractExecutorService {
    ...
    private bool compareAndIncrementWorkerCount(int expect) {
        return AtomicHelper.cas(ctl, expect, expect + 1);
    }
    ...
    private void advanceRunState(int targetState) {
        ...
        for (;;) {
            ...
            if (runStateAtLeast(c, targetState) ||
                AtomicHelper.cas(ctl, c, ctlOf(targetState, workerCountOf(c))))
                break;
        }
    }
    ...
    final void tryTerminate() {
        for (;;) {
            ...
            mainLock.lock();
            try {
                if (AtomicHelper.cas(ctl, c, ctlOf(TIDYING, 0))) {
                    try {
                        terminated();
                    } finally {
