module hunt.security.util.DerInputStream;

import hunt.security.util.DerIndefLenConverter;
...
class DerInputStream {
     ...
     */
    this(byte[] data) {
        initilize(data, 0, data.length, true);
    }
    ...
    this(byte[] data, int offset, int len,
        bool allowBER) {
        initilize(data, offset, len, allowBER);
    }
     ...
     */
    this(byte[] data, int offset, int len) {
        initilize(data, offset, len, true);
    }

    /*
     * private helper routine
     */
    private void initilize(byte[] data, int offset, size_t len, bool allowBER) {
        ...
        if (DerIndefLenConverter.isIndefinite(data[offset+1])) {
            if (!allowBER) {
                ...
                byte[] inData = data[offset .. offset+len].dup;
    ...
    protected DerValue[] readVector(int startLen) {
        ...
        byte lenByte = cast(byte)buffer.read();
        ...
        //  */
        // Array!DerValue vec; // = new Vector<DerValue>(startLen);
        // DerValue value;

        // do {
        //     value = new DerValue(newstr.buffer, buffer.allowBER);
        //     vec.insertBack(value);
     ...
     */
    static int getLength(InputStream stream) {
        return getLength(stream.read(), stream);
    }
     ...
     * @exception IOException on parsing error or unsupported lengths.
     */
    static int getLength(int lenByte, InputStream stream) {
        int value, tmp;
        if (lenByte == -1) {
        ...
        if ((tmp & 0x080) == 0x00) { // short form, 1 byte datum
            ...
            if (tmp == 0)
                return -1;
            if (tmp < 0 || tmp > 4)
                throw new IOException(mdName ~ "lengthTag=" ~ tmp.to!string() ~ ", "
                    ~ ((tmp < 0) ? "incorrect DER encoding." : "too big."));

            value = 0x0ff & stream.read();
            tmp--;
            if (value == 0) {
     ...
     */
    void mark(int value) { buffer.mark(value); }
