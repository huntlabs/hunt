 ...
 * For example, the value "2nd October 2007" can be stored _in a {@code LocalDate}.
 * !(p)
...
public  class LocalDate
     ...
     */
    enum long DAYS_0000_TO_1970 = (DAYS_PER_CYCLE * 5L) - (30L * 365L + 7L);
    ...
    //-----------------------------------------------------------------------
    /**
     * Obtains the current date from the system clock _in the default time-zone.
     * !(p)
    ...
    /**
     * Obtains the current date from the system clock _in the specified time-zone.
     * !(p)
    ...
    public static LocalDate ofYearDay(int year, int dayOfYear) {
        ...
        if (dayOfYear == 366 && leap == false) {
            throw new DateTimeException("Invalid date 'DayOfYear 366' as '" ~ year.to!string ~ "' is not a leap year");
        }
     ...
     * {@link hunt.time.format.DateTimeFormatter#ISO_LOCAL_DATE}.
     *
    ...
    private static LocalDate create(int year, int month, int dayOfMonth) {
        if (dayOfMonth > 28) {
            ...
            if (dayOfMonth > dom) {
                if (dayOfMonth == 29) {
                    throw new DateTimeException("Invalid date 'February 29' as '" ~ year.to!string ~ "' is not a leap year");
                } else {
     ...
     */
    private this(int year, int month, int dayOfMonth) {
        this.year = year;
        this.month = cast(short) month;
        this.day = cast(short) dayOfMonth;
    }
    ...
    public bool isSupported(TemporalField field) {
        return /* ChronoLocalDate. super.*/super_isSupported(field);
    }
    bool super_isSupported(TemporalField field) {
    ...
    public bool isSupported(TemporalUnit unit) {
        return /* ChronoLocalDate.super. */super_isSupported(unit);
    }
    ...
    public ValueRange range(TemporalField field) {
        if (cast(ChronoField)(field) !is null) {
            ...
            if (f.isDateBased()) {
                ...
                {
                    if(name == ChronoField.DAY_OF_MONTH.toString) return ValueRange.of(1, lengthOfMonth());
                    if(name == ChronoField.DAY_OF_YEAR.toString) return ValueRange.of(1, lengthOfYear());
                    if(name == ChronoField.ALIGNED_WEEK_OF_MONTH.toString) return ValueRange.of(1, getMonth() == Month.FEBRUARY && isLeapYear() == false ? 4 : 5);
                    if(name == ChronoField.YEAR_OF_ERA.toString)
    ...
    public int get(TemporalField field) {
        ...
        return /* ChronoLocalDate. super.*/super_get(field);
    }
     int super_get(TemporalField field) {
        ValueRange range = range(field);
        if (range.isIntValue() == false) {
            throw new UnsupportedTemporalTypeException("Invalid field " ~ range.toString ~ " for get() method, use getLong() instead");
        }
        long value = getLong(field);
        if (range.isValidValue(value) == false) {
            throw new DateTimeException("Invalid value for " ~ typeid(field).stringof ~ " (valid values " ~ range.toString ~ "): " ~ value.to!string);
        }
    ...
    private int get0(TemporalField field) {
        auto f = cast(ChronoField) field;
         {
            if(f ==  ChronoField.DAY_OF_WEEK) return getDayOfWeek().getValue();
            if(f ==  ChronoField.ALIGNED_DAY_OF_WEEK_IN_MONTH) return ((day - 1) % 7) + 1;
            if(f ==  ChronoField.ALIGNED_DAY_OF_WEEK_IN_YEAR) return ((getDayOfYear() - 1) % 7) + 1;
            if(f ==  ChronoField.DAY_OF_MONTH) return day;
            if(f ==  ChronoField.DAY_OF_YEAR) return getDayOfYear();
            if(f ==  ChronoField.EPOCH_DAY) throw new UnsupportedTemporalTypeException("Invalid field 'EpochDay' for get() method, use getLong() instead");
            if(f ==  ChronoField.ALIGNED_WEEK_OF_MONTH) return ((day - 1) / 7) + 1;
            if(f ==  ChronoField.ALIGNED_WEEK_OF_YEAR) return ((getDayOfYear() - 1) / 7) + 1;
            if(f ==  ChronoField.MONTH_OF_YEAR) return month;
            if(f ==  ChronoField.PROLEPTIC_MONTH) throw new UnsupportedTemporalTypeException("Invalid field 'ProlepticMonth' for get() method, use getLong() instead");
            if(f ==  ChronoField.YEAR_OF_ERA) return (year >= 1 ? year : 1 - year);
            if(f ==  ChronoField.YEAR) return year;
            if(f ==  ChronoField.ERA) return (year >= 1 ? 1 : 0);
        }
     ...
     *
     *  result = localDate._with(JULY)._with(lastDayOfMonth());
     * </pre>
     * !(p)
     ...
     */
    override
    public LocalDate _with(TemporalAdjuster adjuster) {
        // optimizations
        if (cast(LocalDate)(adjuster) !is null) {
            return cast(LocalDate) adjuster;
        }
        return cast(LocalDate) adjuster.adjustInto(this);
    }
     ...
     *  Aligned weeks are counted such that the first week of a given month starts
     *  on the first day of that month.
     *  This adjustment moves the date _in whole week chunks to match the specified week.
     ...
     */
    override
    public LocalDate _with(TemporalField field, long newValue) {
        if (cast(ChronoField)(field) !is null) {
            ChronoField f = cast(ChronoField) field;
            f.checkValidValue(newValue);
             {
                if( f == ChronoField.DAY_OF_WEEK) return plusDays(newValue - getDayOfWeek().getValue());
                if( f == ChronoField.ALIGNED_DAY_OF_WEEK_IN_MONTH) return plusDays(newValue - getLong(ChronoField.ALIGNED_DAY_OF_WEEK_IN_MONTH));
                if( f == ChronoField.ALIGNED_DAY_OF_WEEK_IN_YEAR) return plusDays(newValue - getLong(ChronoField.ALIGNED_DAY_OF_WEEK_IN_YEAR));
                if( f == ChronoField.DAY_OF_MONTH) return withDayOfMonth(cast(int) newValue);
                if( f == ChronoField.DAY_OF_YEAR) return withDayOfYear(cast(int) newValue);
                if( f == ChronoField.EPOCH_DAY) return LocalDate.ofEpochDay(newValue);
                if( f == ChronoField.ALIGNED_WEEK_OF_MONTH) return plusWeeks(newValue - getLong(ChronoField.ALIGNED_WEEK_OF_MONTH));
                if( f == ChronoField.ALIGNED_WEEK_OF_YEAR) return plusWeeks(newValue - getLong(ChronoField.ALIGNED_WEEK_OF_YEAR));
                ...
                if( f == ChronoField.YEAR_OF_ERA) return withYear(cast(int) (year >= 1 ? newValue : 1 - newValue));
        ...
        return cast(LocalDate)(field.adjustInto(this, newValue));
    }
    ...
    public LocalDate plus(TemporalAmount amountToAdd) {
        ...
        return cast(LocalDate) amountToAdd.addTo(this);
    }
    ...
    public LocalDate plus(long amountToAdd, TemporalUnit unit) {
        ...
        return cast(LocalDate)(unit.addTo(this, amountToAdd));
    }
    ...
    public LocalDate plusDays(long daysToAdd) {
        ...
        if (dom > 0) {
            if (dom <= 28) {
                return new LocalDate(year, month, cast(int) dom);
            } else if (dom <= 59) { // 59th Jan is 28th Feb, 59th Feb is 31st Mar
                long monthLen = lengthOfMonth();
                if (dom <= monthLen) {
                    return new LocalDate(year, month, cast(int) dom);
                } else if (month < 12) {
                    return new LocalDate(year, month + 1, cast(int) (dom - monthLen));
                } else {
                    ChronoField.YEAR.checkValidValue(year + 1);
                    return new LocalDate(year + 1, 1, cast(int) (dom - monthLen));
                }
            }
        }
    ...
    public LocalDate minus(TemporalAmount amountToSubtract) {
        ...
        return cast(LocalDate) amountToSubtract.subtractFrom(this);
    }
    ...
    public Temporal adjustInto(Temporal temporal) {
        return /* ChronoLocalDate. super.*/super_adjustInto(temporal);
    }

     Temporal super_adjustInto(Temporal temporal) {
        return temporal._with(ChronoField.EPOCH_DAY, toEpochDay());
    }
     ...
     * For example, the amount _in months between 2012-06-15 and 2012-08-14
     ...
     * !(p)
     * The n-th date which appears _in the stream is equal to {@code this.plus(step.multipliedBy(n))}
     ...
     */
    // override
    public LocalDateTime atTime_s(LocalTime time) {
        return LocalDateTime.of(this, time);
    }
    override 
     ChronoLocalDateTime!(ChronoLocalDate) atTime(LocalTime localTime) {
        return ChronoLocalDateTimeImpl!ChronoLocalDate.of!ChronoLocalDate(this, localTime);
    }
    /**
     * Combines this date with a time to create a {@code LocalDateTime}.
     * !(p)
    ...
    public LocalDateTime atTime(int hour, int minute) {
        return atTime_s(LocalTime.of(hour, minute));
    }
    ...
    public LocalDateTime atTime(int hour, int minute, int second) {
        return atTime_s(LocalTime.of(hour, minute, second));
    }
    ...
    public LocalDateTime atTime(int hour, int minute, int second, int nanoOfSecond) {
        return atTime_s(LocalTime.of(hour, minute, second, nanoOfSecond));
    }
     ...
     * to the rules _in the time-zone.
     * !(p)
     ...
     * !(p)
     * To convert to a specific time _in a given time-zone call {@link #atTime(LocalTime)}
    ...
    public ZonedDateTime atStartOfDay(ZoneId zone) {
        ...
        // standard ZDT factory would result _in 01:00 rather than 00:30
        LocalDateTime ldt = atTime_s(LocalTime.MIDNIGHT);
        if ((cast(ZoneOffset)(zone) !is null) == false) {
            ZoneRules rules = zone.getRules();
    ...
    public long toEpochSecond(LocalTime time, ZoneOffset offset) {
        ...
        long secs = toEpochDay() * LocalTime.SECONDS_PER_DAY + time.toSecondOfDay();
     ...
     * chronology is also considered, see {@link hunt.time.chrono.ChronoLocalDate#compareTo}.
     *
    ...
    public int compareTo(ChronoLocalDate other) {
        if (cast(LocalDate)(other) !is null) {
            return compareTo0(cast(LocalDate) other);
        }
        return /* ChronoLocalDate.super.*/super_compareTo(other);
    }
     int super_compareTo(ChronoLocalDate other) {
        int cmp = compare(toEpochDay(), other.toEpochDay());
        if (cmp == 0) {
    ...
    override  // override for Javadoc and performance
    public int opCmp(ChronoLocalDate other) {
        if (cast(LocalDate)(other) !is null) {
            return compareTo0(cast(LocalDate) other);
        }
        return /* ChronoLocalDate.super.*/super_compareTo(other);
     ...
     */
    override  // override for Javadoc and performance
    public bool isAfter(ChronoLocalDate other) {
        if (cast(LocalDate)(other) !is null) {
            return compareTo0(cast(LocalDate) other) > 0;
        }
        return /* ChronoLocalDate. super.*/super_isAfter(other);
    }

     bool super_isAfter(ChronoLocalDate other) {
     ...
     */
    override  // override for Javadoc and performance
    public bool isBefore(ChronoLocalDate other) {
        if (cast(LocalDate)(other) !is null) {
            return compareTo0(cast(LocalDate) other) < 0;
        }
        return /* ChronoLocalDate. super.*/super_isBefore(other);
    }

     bool super_isBefore(ChronoLocalDate other) {
        return this.toEpochDay() < other.toEpochDay();
    }
     ...
     */
    override  // override for Javadoc and performance
    public bool isEqual(ChronoLocalDate other) {
        if (cast(LocalDate)(other) !is null) {
            return compareTo0(cast(LocalDate) other) == 0;
        }
        return /* ChronoLocalDate. super.*/super_isEqual(other);
    }
     bool super_isEqual(ChronoLocalDate other) {
        return this.toEpochDay() == other.toEpochDay();
    }
     ...
     * !(p)
     * The output will be _in the ISO-8601 format {@code uuuu-MM-dd}.
     *
     ...
     * @serialData
     * !(pre)
     *  _out.writeByte(3);  // identifies a LocalDate
     *  _out.writeInt(year);
    ...
    static LocalDate readExternal(DataInput _in) /*throws IOException*/ {
