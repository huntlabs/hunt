...
public interface Temporal : TemporalAccessor {
     ...
     * !(pre)
     *  date = date._with(Month.JULY);        // most key classes implement TemporalAdjuster
     *  date = date._with(lastDayOfMonth());  // static import from Adjusters
     *  date = date._with(next(WEDNESDAY));   // static import from Adjusters and DayOfWeek
     * </pre>
     *
     * @implSpec
     * !(p)
     ...
     * The amount is typically a {@link hunt.time.Period} but may be any other type implementing
     * the {@link TemporalAmount} interface, such as {@link hunt.time.Duration}.
     * !(p)
     * Some example code indicating how and why this method is used:
     * !(pre)
    ...
    //-----------------------------------------------------------------------
    /**
     * Calculates the amount of time until another temporal _in terms of the specified unit.
     * !(p)
     * This calculates the amount of time between two temporal objects
     * _in terms of a single {@code TemporalUnit}.
     ...
     * the second argument.
     * !(p)
     * In summary, implementations must behave _in a manner equivalent to this pseudo-code:
     * !(pre)
     ...
     *  _in terms of the unit; positive if the specified object is later than this one,
