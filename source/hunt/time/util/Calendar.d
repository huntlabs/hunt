...
public abstract class Calendar : Serializable, Cloneable, Comparable!(Calendar) {
     ...
     * This is an array of <code>FIELD_COUNT</code> bools, with index values
     * <code>ERA</code> through <code>DST_OFFSET</code>.
     * @serial
     */
    //@SuppressWarnings("ProtectedField")
     ...
     */
    private enum int        MINIMUM_USER_STAMP = 2;

    /**
     * The mask value that represents all of the fields.
     */
    enum int ALL_FIELDS = (1 << FIELD_COUNT) - 1;
    ...
    //     areFieldsSet, and isTimeSet become , and isSet[] is
    //     removed. In JDK 1.1.6 we write a format compatible with version 2.
    enum int        currentSerialVersion = 1;
    ...
    enum int YEAR_MASK          = (1 << YEAR);
    enum int MONTH_MASK         = (1 << MONTH);
    enum int WEEK_OF_YEAR_MASK  = (1 << WEEK_OF_YEAR);
    enum int WEEK_OF_MONTH_MASK = (1 << WEEK_OF_MONTH);
    enum int DAY_OF_MONTH_MASK  = (1 << DAY_OF_MONTH);
    enum int DATE_MASK          = DAY_OF_MONTH_MASK;
    enum int DAY_OF_YEAR_MASK   = (1 << DAY_OF_YEAR);
    enum int DAY_OF_WEEK_MASK   = (1 << DAY_OF_WEEK);
    enum int DAY_OF_WEEK_IN_MONTH_MASK  = (1 << DAY_OF_WEEK_IN_MONTH);
    enum int AM_PM_MASK         = (1 << AM_PM);
    enum int HOUR_MASK          = (1 << HOUR);
    enum int HOUR_OF_DAY_MASK   = (1 << HOUR_OF_DAY);
    enum int MINUTE_MASK        = (1 << MINUTE);
    enum int SECOND_MASK        = (1 << SECOND);
    enum int MILLISECOND_MASK   = (1 << MILLISECOND);
    enum int ZONE_OFFSET_MASK   = (1 << ZONE_OFFSET);
    enum int DST_OFFSET_MASK    = (1 << DST_OFFSET);
     ...
     * {@linkplain #setLenient(bool) leniency mode} parameters can be set.
     *
     * <p><b>Examples</b>
    ...
    //     private bool isInstantSet() {
    //         return nextStamp == COMPUTED;
    //     }

    //     private bool isSet(int index) {
    //         return fields !is null && fields[index] > UNSET;
    //     }

    //     private bool isValidWeekParameter(int value) {
    ...
    //  * @see #get(int)
    //  */
    // protected const int internalGet(int field)
    // {
    //     return fields[field];
    // }
    ...
    //  * @see #set(int,int)
    //  */
    // const void internalSet(int field, int value)
    // {
    //     fields[field] = value;
    // }
    ...
    // bool checkDisplayNameParams(int field, int style, int minStyle, int maxStyle,
    ...
    //  */
    // const bool isExternallySet(int field) {
    ...
    //  * @see #selectFields()
    //  */
    // const void setFieldsComputed(int fieldMask) {
    ...
    //  * @see #selectFields()
    //  */
    // const void setFieldsNormalized(int fieldMask) {
    //     if (fieldMask != ALL_FIELDS) {
    //         for (int i = 0; i < fields.length; i++) {
    //             if ((fieldMask & 1) == 0) {
    //                 stamp[i] = fields[i] = 0; // UNSET == 0
    ...
    //  */
    // const bool isPartiallyNormalized() {
    ...
    //  * version of {@link #roll(int,bool) roll()} that rolls by one unit.  This may not
    ...
    //  */
    // void setZoneShared(bool shared) {
    //     sharedZone = shared;
    // }
    ...
    //         other.isSet = new bool[FIELD_COUNT];
    ...
    // private enum String[] FIELD_NAME = {
    ...
    //     if (savedZone !is null) {
    //         zone = savedZone;
    //     }
    // }

    // private static class CalendarAccessControlContext {
    //     private enum AccessControlContext INSTANCE;
    //     static {
    ...
    // {
    //     const ObjectInputStream input = stream;
    ...
    //         if (tz !is null && tz.hasSameRules(zone) && tz.getID().equals(id)) {
    //             zone = tz;
    //         }
    //     }
    // }
