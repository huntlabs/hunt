module hunt.util.character.CharacterDataLatin1;

import hunt.util.character.Character;
import hunt.util.character.CharacterData;
...
class CharacterDataLatin1 : CharacterData {
    ...
    override int getProperties(int ch) {
        char offset = cast(char)ch;
        int props = A[offset];
        return props;
    }
    ...
    override bool isOtherLowercase(int ch) {
        int props = getPropertiesEx(ch);
        return (props & 0x0001) != 0;
    }

    override bool isOtherUppercase(int ch) {
        int props = getPropertiesEx(ch);
        return (props & 0x0002) != 0;
    }

    override bool isOtherAlphabetic(int ch) {
        int props = getPropertiesEx(ch);
        return (props & 0x0004) != 0;
    }

    override bool isIdeographic(int ch) {
        int props = getPropertiesEx(ch);
        return (props & 0x0010) != 0;
    }

    override int getType(int ch) {
        int props = getProperties(ch);
        return (props & 0x1F);
    }

    override bool isJavaIdentifierStart(int ch) {
        int props = getProperties(ch);
        return ((props & 0x00007000) >= 0x00005000);
    }

    override bool isJavaIdentifierPart(int ch) {
        int props = getProperties(ch);
        return ((props & 0x00003000) != 0);
    }

    override bool isUnicodeIdentifierStart(int ch) {
        int props = getProperties(ch);
        return ((props & 0x00007000) == 0x00007000);
    }

    override bool isUnicodeIdentifierPart(int ch) {
        int props = getProperties(ch);
        return ((props & 0x00001000) != 0);
    }

    override bool isIdentifierIgnorable(int ch) {
        int props = getProperties(ch);
        return ((props & 0x00007000) == 0x00001000);
    }

    override int toLowerCase(int ch) {
        int mapChar = ch;
    ...
    override int toUpperCase(int ch) {
        int mapChar = ch;
        int val = getProperties(ch);
    ...
    override int toTitleCase(int ch) {
    ...
    override int getNumericValue(int ch) {
        int val = getProperties(ch);
        int retval = -1;
    ...
    override bool isWhitespace(int ch) {
        int props = getProperties(ch);
        return ((props & 0x00007000) == 0x00004000);
    }

    override byte getDirectionality(int ch) {
        int val = getProperties(ch);
        byte directionality = cast(byte)((val & 0x78000000) >> 27);
    ...
    override bool isMirrored(int ch) {
        int props = getProperties(ch);
        return ((props & 0x80000000) != 0);
    }

    override int toUpperCaseEx(int ch) {
        int mapChar = ch;
        int val = getProperties(ch);
    ...
    enum char[] sharpsMap = ['S', 'S'];

    override char[] toUpperCaseCharArray(int ch) {
        char[] upperMap = [cast(char)ch];
        if (ch == 0x00DF) {
            upperMap = sharpsMap;
        }
        return upperMap;
    ...
    __gshared static CharacterDataLatin1 instance;
