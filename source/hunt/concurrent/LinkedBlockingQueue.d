...
class LinkedBlockingQueue(E) : AbstractQueue!(E), BlockingQueue!(E) {
    ...
    //         try {
    //             E e = null;
    //             for (p = p.next; p !is null && (e = p.item) is null; )
    //                 p = succ(p);
    //             next = p;
    //             nextItem = e;
    //         } finally {
    ...
    //                 } else
    //                     n = 0;
    //                 for (; p !is null && n < len; p = succ(p))
    //                     if ((es[n] = p.item) !is null) {
    //                         lastRet = p;
    //                         n++;
    //                     }
    //             } finally {
     ...
     */
    // private final class LBQSpliterator : Spliterator!(E) {
    //     static int MAX_BATCH = 1 << 25;  // max batch array size;
    //     Node!(E) current;    // current node; null until initialized
    //     int batch;          // batch size for splits
    //     bool exhausted;  // true when no more nodes
    //     long est = size();  // size estimate

    //     this() {}

    //     long estimateSize() { return est; }

    //     Spliterator!(E) trySplit() {
    //         Node!(E) h;
    //         if (!exhausted &&
    //             ((h = current) !is null || (h = head.next) !is null)
    //             && h.next !is null) {
    ...
    //             fullyLock();
    //             try {
    //                 if (p !is null || (p = head.next) !is null)
    //                     for (; p !is null && i < n; p = succ(p))
    //                         if ((a[i] = p.item) !is null)
    //                             i++;
    //             } finally {
    //                 fullyUnlock();
    //             }
    //             if ((current = p) is null) {
    //                 est = 0L;
    //                 exhausted = true;
    //             }
    //             else if ((est -= i) < 0L)
    ...
    //     bool tryAdvance(Consumer!(E) action) {
    //         Objects.requireNonNull(action);
    //         if (!exhausted) {
    //             E e = null;
    //             fullyLock();
    //             try {
    //                 Node!(E) p;
    //                 if ((p = current) !is null || (p = head.next) !is null)
    //                     do {
    //                         e = p.item;
    //                         p = succ(p);
    //                     } while (e is null && p !is null);
    //                 if ((current = p) is null)
    //                     exhausted = true;
    //             } finally {
    //                 fullyUnlock();
