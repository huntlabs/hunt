 ...
 * @since   1.0
 */
class BitSet  { // : Cloneable, java.io.Serializable
    /*
     * BitSets are packed into arrays of "words."  Currently a word is
     * a long, which consists of 64 bits, requiring 6 address bits.
     * The choice of word size is determined purely by performance concerns.
     */
    private enum int ADDRESS_BITS_PER_WORD = 6;
    private enum int BITS_PER_WORD = 1 << ADDRESS_BITS_PER_WORD;
    private enum int BIT_INDEX_MASK = BITS_PER_WORD - 1;

    /* Used to shift left or right for a partial word mask */
    private enum long WORD_MASK = 0xffffffffffffffffL;
     ...
     */
    private static size_t wordIndex(size_t bitIndex) {
        return bitIndex >> ADDRESS_BITS_PER_WORD;
    }

    /**
     * Every method must preserve these invariants.
     */
    ...
    this(size_t nbits) {
        ...
        if (nbits < 0)
            throw new NegativeArraySizeException("nbits < 0: " ~ nbits.to!string());

        initWords(nbits);
        sizeIsSticky = true;
    }

    private void initWords(size_t nbits) {
    ...
    */
     ...
     */
    private void ensureCapacity(size_t wordsRequired) {
     ...
     */
    private void expandTo(size_t wordIndex) {
     ...
     */
    private static void checkRange(size_t fromIndex, size_t toIndex) {
        if (fromIndex < 0)
            throw new IndexOutOfBoundsException("fromIndex < 0: " ~ fromIndex.to!string());
        if (toIndex < 0)
            throw new IndexOutOfBoundsException("toIndex < 0: " ~ toIndex.to!string());
        if (fromIndex > toIndex)
            throw new IndexOutOfBoundsException("fromIndex: " ~ fromIndex.to!string() ~
                                                " > toIndex: " ~ toIndex.to!string());
    }
    ...
    void flip(size_t bitIndex) {
        if (bitIndex < 0)
            throw new IndexOutOfBoundsException("bitIndex < 0: " ~ bitIndex.to!string());
     ...
     *
     * @param  bitIndex a bit index
     * @param  value a bool value to set
     * @throws IndexOutOfBoundsException if the specified index is negative
     * @since  1.4
     */
    ...
    void set(int fromIndex, int toIndex) {
        ...
        size_t startWordIndex = wordIndex(fromIndex);
        size_t endWordIndex   = wordIndex(toIndex - 1);
        expandTo(endWordIndex);
    ...
    BitSet get(size_t fromIndex, size_t toIndex) {
        ...
        size_t targetWords = wordIndex(toIndex - fromIndex - 1) + 1;
        size_t sourceIndex = wordIndex(fromIndex);
        bool wordAligned = ((fromIndex & BIT_INDEX_MASK) == 0);

        // Process all words but the last word
        for (size_t i = 0; i < targetWords - 1; i++, sourceIndex++)
    ...
    size_t nextClearBit(int fromIndex) {
        ...
        while (true) {
            if (word != 0)
                return (u * BITS_PER_WORD) + Long.numberOfTrailingZeros(word);
            if (++u == wordsInUse)
                return cast(int)wordsInUse * BITS_PER_WORD;
            word = ~words[u];
        }
    }
    ...
    size_t previousSetBit(size_t fromIndex) {
        if (fromIndex < 0) {
            ...
            throw new IndexOutOfBoundsException(
                "fromIndex < -1: " ~ fromIndex.to!string());
        }
     ...
     * @return bool indicating whether this {@code BitSet} intersects
     *         the specified {@code BitSet}
     * @since  1.4
     */
    bool intersects(BitSet set) {
        for (size_t i = min(wordsInUse, set.wordsInUse) - 1; i >= 0; i--)
    ...
    void or(BitSet set) {
        ...
        size_t wordsInCommon = min(wordsInUse, set.wordsInUse);
        ...
        for (size_t i = 0; i < wordsInCommon; i++)
            words[i] |= set.words[i];
    ...
    override size_t toHash() @trusted nothrow {
        long h = 1234;
        for (size_t i = wordsInUse; --i >= 0; )
            h ^= words[i] * (i + 1);

        return cast(size_t)((h >> 32) ^ h);
    }
    ...
    //         private bool root; // true if root and not split
    ...
    //         BitSetSpliterator(int origin, int fence, int est, bool root) {
    //             this.index = origin;
    //             this.fence = fence;
    //             this.est = est;
    //             this.root = root;
