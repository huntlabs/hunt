module hunt.system.TimeZone;

import std.string;

string getSystemTimeZoneId(string homeDir = "") {
    version (Posix) {
        return findTZ_md(homeDir);
...
version (Posix) {
    ...
    string findTZ_md(string home_dir) {
        ...
        return cast(string)fromStringz(javatz); 
    }
...
version (Windows) {
    ...
    */
    static string[] keyNames = [
        ("StandardName"), ("StandardName"), ("Std"), ("Std")
    ];
    ...
    * returned.
    */
    private static char* matchJavaTZ(const char* home_dir, char* tzName) {
        int line;
        int IDmatched = 0;
        FILE* fp;
    ...
    string findTZ_md(string home_dir) {
        ...
        if (result != VALUE_UNKNOWN) {
            if (result == VALUE_GMTOFFSET) {
                ...
                std_timezone = matchJavaTZ(home_dir.toStringz(), winZoneName.ptr);
                if (std_timezone == null) {
                    std_timezone = getGMTOffsetID();
                }
            }
        }
        return cast(string)fromStringz(std_timezone);
    }
